...
class OrderServices extends Base{
    ...
    public function crontab(){
        ...
        foreach($rs as $k=>$v){
            ...
            if($v['goodsServiceType']==0){
                ...
                // 生成退款订单
                $this->makeRefundData($v['orderId'], $v['refundMoney'], $v['id']);
            }
            Db::name('log_services')->insertAll($logs);
        }
        unset($k, $v);
    ...
    private function makeRefundData($orderId, $refundMoney, $serviceId){
        ...
        //修改商家未结算金额
        $order = Db::name("orders")->where(["orderId"=>$orderId])->field("orderId,shopId")->find();
        $shopId = $order["shopId"];
        $where = [];
    ...
    public function getRefundableMoney($uId=0, $ids='', $numArr=[]){
        ...
        $rs = ['totalMoney'=>0,'totalScore'=>0,'useScoreMoney'=>0,'getScoreMoney'=>0];
        $ids = input('ids',$ids);
        // 未传入og表主键
                ...
                if($currSGoodsNum<=($v['goodsNum']-(int)$currSGoods['totalNum'])){
                    ...
                    if($v['goodsNum']==1){ //只有一件
                        ...
                        $rs['totalScore'] = $v['useScoreVal'];
                            ...
                            if($isScorePay)$rs['totalScore'] = $v['useScoreVal'] - $v['getScoreVal'];
                        ...
                        for($i=1;$i<=$currSGoodsNum;++$i){
                            if($i==$lastIndex){// 最后一件
                                ...
                                $rs['totalScore'] += $lastScoreVal;
                                    // 若为纯积分支付，则 可退还积分需要减去下单获得的积分
                                    if($isScorePay)$rs['totalScore'] = $rs['totalScore'] - $lastGetScoreVal;
                                ...
                                $rs['totalScore'] += $avgScoreVal;
                                    // 若为纯积分支付，则 可退还积分需要减去下单获得的积分
                                    if($isScorePay)$rs['totalScore'] = $rs['totalScore'] - $avgGetScoreVal;
