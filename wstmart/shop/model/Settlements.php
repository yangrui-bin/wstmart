...
class Settlements extends Base{
  protected $pk = 'settlementId';
    ...
    public function pageSettledQuery(){
        ...
        $page = Db::name('orders')->alias('o')->join('__SETTLEMENTS__ s','o.settlementId=s.settlementId')->where($where)->field('orderId,orderNo,payType,goodsMoney,deliverMoney,totalMoney,o.commissionFee,realTotalMoney,s.settlementTime,s.settlementNo')->order('s.settlementTime desc')->paginate(input('limit/d'))->toArray();
        if(count($page['data'])){
            foreach ($page['data'] as $key => $v) {
