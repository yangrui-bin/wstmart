   ...
   public function pageShopOrderQuery(){
         ...
         if(in_array($payType,[0,1]))$where[] = ['payType','=',$payType];
   	     $page = Db::name('orders')->where($where)
        	          ->field('orderId,orderNo,payType,goodsMoney,deliverMoney,realTotalMoney,totalMoney,commissionFee,createTime,scoreMoney')
        ...
        if(count($page['data'])>0){
        	foreach ($page['data'] as $key => $v) {
                ...
                if($v['payType']==1){
                     //在线支付的返还金额=实付金额+积分抵扣金额-佣金
                     $backMoney = $v['realTotalMoney']+$v['scoreMoney']-$v['commissionFee'];
                }else{
                     //货到付款的返还金额=积分抵扣金额-佣金
